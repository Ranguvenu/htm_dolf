...
class core_webservice_external extends external_api {
    ...
    public static function get_site_info($serviceshortnames = array()) {
        ...
        $roleinfo = $DB->get_record_sql('SELECT rl.id,rl.shortname,rl.name FROM {role} rl
                                              ...
                                              AND rol.contextid = :contextid',['userid'=>$USER->id,'contextid'=>$systemcontext->id]);          
        $siteinfo['role']['id'] = $roleinfo->id;
        $siteinfo['role']['shortname'] = $roleinfo->shortname;
        $siteinfo['role']['name']= $roleinfo->name;
