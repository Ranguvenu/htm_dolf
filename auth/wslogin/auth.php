...
use auth_wslogin\core_wslogin_manager;
use auth_wslogin\wslogin_manager_interface;
 ...
 */
class auth_plugin_wslogin extends auth_plugin_base {
     ...
     * User key manager.
     *
     * @var wslogin_manager_interface
     */
    protected $wsloginmanager;
    ...
    public function __construct() {
        $this->authtype = 'wslogin';
        $this->config = get_config('auth_wslogin');
        $this->wsloginmanager = new core_wslogin_manager($this->config);
    }
    ...
    protected function redirect($url) {
        if (CLI_SCRIPT or AJAX_SCRIPT) {
            throw new moodle_exception('redirecterrordetected', 'auth_wslogin', '', $url);
        }
    ...
    public function user_login($username, $password) {
        ...
        $fa = new auth_wslogin\fa($username, $password);
    ...
    private function map_data($data){
        $userrecord = array();
        $userrecord['firstname'] = ucfirst($data->firstNameEn);
        $userrecord['lastname'] =ucfirst($data->lastNameEn);
        $userrecord['firstnamearabic'] = $data->firstNameAr;
        $userrecord['lastnamearabic'] =$data->lastNameAr;
        $userrecord['gender'] = $data->gender;
        $userrecord['lang'] = 'ar';
        $userrecord['nationality'] = $data->nationalityCountryId;
        $userrecord['id_type'] = $data->id_type;
        $userrecord['id_number'] = $data->idNumber;
  
        $userrecord['username'] = $data->username;
        $userrecord['email'] = strtolower($data->email);
        $userrecord['password'] = 'testes';
        $userrecord['phone1'] = $data->phone;
  
        $userrecord['timecreated'] =time();
        $userrecord['usercreated'] =$USER->id;
        $userrecord['approvedstatus'] = 2;
        $userrecord['authmethod'] = 'wslogin';
        return $userrecord;
    }

    /**
     * Logs a user in using wslogin and redirects after.
     *
     * @TODO: refactor this method to make it easy to read.
     *
     * @throws \moodle_exception If something went wrong.
     */
    public function user_login_wslogin() {
        ...
        try {
            $key = $this->wsloginmanager->validate_key($keyvalue);
        ...
        if (isloggedin()) {
            if ($USER->id != $key->userid) {
                ...
                $this->wsloginmanager->delete_keys($key->userid);
     ...
     *
     * @param \auth_wslogin\wslogin_manager_interface $keymanager
     */
    public function set_wslogin_manager(wslogin_manager_interface $keymanager) {
        $this->wsloginmanager = $keymanager;
    }
    ...
    protected function get_user(array $data) {
        ...
        if (empty($user)) {
            if ($this->should_create_user()) {
                    ...
                    $user->id = (new \local_userapproval\action\manageuser)->create_user($data);
                    $userid = $user->id;
                    $user = (new \local_userapproval\action\manageuser)->create_custom_user($data,$user->id);
            ...
            $data->id = $user->id;
            $updatecustomrecord = (new \local_userapproval\action\manageuser)->update_custom_user($data,$user->id);
            $user = (new \local_userapproval\action\manageuser)->user_update_user($data,$user->id);
        }
        $systemcontext = context_system::instance();
        role_assign($traineeroleid, $userid, $systemcontext);
    ...
    protected function generate_user_key(array $data) {
        ...
        return $this->wsloginmanager->create_key($user->id, $ips);
    }
    ...
    public function get_login_url($data) {
        ...
        $wslogin  = $this->generate_user_key($userdata);

        return $CFG->wwwroot . '/auth/wslogin/login.php?key=' . $wslogin;
    }
     ...
     */
    public function user_logout_wslogin() {
        global $CFG, $USER;

        $redirect = required_param('return', PARAM_URL);

        // We redirect when user's session in Moodle already has expired
        // or the user is still logged in using "wslogin" auth type.
        if (!isloggedin() || $USER->auth == 'wslogin') {
            require_logout();
            $this->redirect($redirect);
        } else {
            // If logged in with different auth type, then display an error.
            print_error('incorrectlogout', 'auth_wslogin', $CFG->wwwroot);
        }
    }
}
