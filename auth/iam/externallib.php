...
require_once($CFG->dirroot . "/auth/iam/auth.php");

class auth_iam_external extends external_api {
        ...
        return new external_function_parameters(
            array(
                'user' => new external_single_structure(
                    get_auth_plugin('iam')->get_request_login_url_user_parameters()
                )
            )
        );
    }
    ...
    public static function request_login_url($user) {
        ...
        if (!is_enabled_auth('iam')) {
            throw new webservice_access_exception(get_string('pluginisdisabled', 'auth_iam'));
        }

        $context = context_system::instance();
        require_capability('auth/iam:generatekey', $context);

        $auth = get_auth_plugin('iam');
