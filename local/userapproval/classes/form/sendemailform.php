...
class sendemailform extends dynamic_form { 
    public function definition() {
        ...
        $mform = $this->_form;
        $userid = $this->optional_param('id',0,PARAM_INT);
        $name = $this->optional_param('name',0,PARAM_RAW);
        $email = $this->optional_param('email',0,PARAM_RAW);
        //echo "useremail My".$email ;
        //exit;
        $mform->addElement('hidden', 'userid', $userid);
        $mform->setType('userid',PARAM_INT);

        /* $record = $DB->get_record('tool_org_order_payments',array('id' => $userid));
       ...
       $fullname = (new \local_trainingprogram\local\trainingprogram())->user_fullname_case();
        ...
        $mform->addElement('static', 'touser', get_string('touser', 'tool_product'), $email);
    ...
    public function process_dynamic_submission() {
        ...
        $data = $this->get_data();
        (new \local_userapproval\action\manageuser)->sendemail_to_orgorind($data);
