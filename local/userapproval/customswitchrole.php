...
$returnurl  = optional_param('returnurl', $CFG->wwwroot, PARAM_RAW);
$systemcontext = context_system::instance();
$returnurl  = clean_param($CFG->wwwroot, PARAM_URL);
$PAGE->set_url('/local/userapproval/customswitchrole.php', array('id'=>$id, 'switchrole'=>$switchrole));
...
if ($switchrole == 0) {
    if ($switchrole == 0 && (!is_siteadmin() && !has_capability('local/organization:manage_trainee',$systemcontext))) {
        $USER->access['rsw'][$context->path] = $SESSION->orole; // Setting Default Role.
    } else {
        $SESSION->orole = 0;
    }

    role_switch($SESSION->orole, $context);
}
