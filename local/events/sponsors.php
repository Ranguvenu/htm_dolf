...
$PAGE->set_url('/local/events/sponsors.php', array('id' => $eventid));
$PAGE->set_context($systemcontext);
$PAGE->set_title(get_string('pluginname','local_events'));
$PAGE->set_heading(get_string('sponsorlist','local_events', $event));
$PAGE->navbar->add(get_string("eventdetails", 'local_events'), new moodle_url("/local/events/view.php?id=".$eventid.""));
$PAGE->navbar->add(get_string('sponsorlist', 'local_events', $event));
echo $OUTPUT->header();
(new local_events\events)->sponsorsinfo($eventid);
echo $OUTPUT->footer();