...
class offering_upload  {
    ...
    protected $allowedcolumns = array('oldid','offeringcode','programcode','offeringstartdate','offeringenddate','offeringstarttime', 'offeringduration', 'offeringtype','seats','organization','sellingprice','actualprice','trainingmethod','meetingtype');
    /**
        ...
        while ($row = $cir->next()) {
            ...
            if (count($this->errors) > 0) { 
               $errorcount++;
                ...
                $programcode = trim($masterdata->programcode);
                $trainingid = $DB->get_field_sql("SELECT id FROM {local_trainingprogram} WHERE code = '$programcode'");

                if(!empty($masterdata->offeringcode) && $DB->record_exists('tp_offerings',array('code' => trim($masterdata->offeringcode)))) {
                    $offeringid = (int) $DB->get_field('tp_offerings','id',array('code' => trim($masterdata->offeringcode)));
                    $masterdata->id = $offeringid;
                    $masterdata->code = trim($masterdata->offeringcode);
                } else {
                    $masterdata->id = 0;
                    $masterdata->code = trim($masterdata->offeringcode);
                }
                ...
                $masterdata->organization  = $masterdata->organization ? $DB->get_field('local_organization','id',['shortname'=>trim($masterdata->organization)]) : 0;
                $masterdata->sellingprice  = $masterdata->sellingprice;
                $masterdata->actualprice  = $masterdata->actualprice;
                $masterdata->trainingmethod  = strtolower($masterdata->trainingmethod);
                ...
                $dur = $masterdata->offeringduration;
                if($dur){
                    $hours = floor($dur / 60);
                    $minutes = ($dur % 60);
                } 

                $masterdata->endtime['hours']  = $star_ttime[0] + $hours;
                $masterdata->endtime['minutes']= $star_ttime[1]+ $minutes;
                $enddate =  $startdate = $data->startdate + ($end_hour * 3600) + ($end_minutes * 60);       
                (new tp)->add_update_schedule_program($masterdata);
                $programrecord=$DB->get_record('local_trainingprogram',array('id'=>$masterdata->trainingid));
                $eventparams = array('context' => \context_system::instance(),'objectid'=>$masterdata->trainingid,'other'=>array('name'=>$programrecord->name));
                $event = \local_trainingprogram\event\row_inserted::create($eventparams);// ... code that may add some record snapshots
                $event->trigger();
    ...
    private function validate_columns() {
        ...
        if (!in_array('offeringcode', $this->columnsmapping)) {
            $this->errors[] = get_string('offeringcode_missing', 'local_trainingprogram');
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringcode'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        ...
        if (!in_array('offeringstartdate', $this->columnsmapping)) {
             ...
             $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringstartdate'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        ...
        if (!in_array('offeringenddate', $this->columnsmapping)) {
             ...
             $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringenddate'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        if (!in_array('offeringstarttime', $this->columnsmapping)) {
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringstarttime'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
         if (!in_array('offeringduration', $this->columnsmapping)) {
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringduration'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        ...
        if (!in_array('offeringtype', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'offeringtype'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        ...
        if (!in_array('organization', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'organization'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
         if (!in_array('sellingprice', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'sellingprice'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        if (!in_array('actualprice', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'actualprice'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
       if (!in_array('trainingmethod', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'trainingmethod'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
        if (!in_array('meetingtype', $this->columnsmapping)) { 
            ...
            $eventparams = array('context' => \context_system::instance(),'other'=>array('name'=>'meetingtype'));
            $event = \local_trainingprogram\event\header_missing::create($eventparams);// ... code that may add some record snapshots
            $event->trigger();
            return  $this->errors;
        }
    ...
    private function required_fields_validations($excel,$option=0){
        ...
        if (array_key_exists('programcode', $excel) ) {
            if (empty($excel->programcode)) {
                ...
                $programcode = trim($excel->programcode);

                $record = $DB->record_exists_sql("SELECT id FROM {local_trainingprogram} WHERE code = '$programcode'");
                ...
                if(!empty(trim($excel->offeringcode))) {
                    $offering_code = trim($excel->offeringcode);
                    $programcode = trim($excel->programcode);
                    $trainingid = $DB->get_field_sql("SELECT id FROM {local_trainingprogram} WHERE code = '$programcode'");
                    $offeringexists = $DB->record_exists_sql("SELECT id FROM {tp_offerings} WHERE code = '$offering_code'");
                    if($offeringexists) {
                        $record = $DB->record_exists_sql("SELECT id FROM {tp_offerings} WHERE code = '$offering_code' AND trainingid = $trainingid");

                        $offeringmappedprogramcode = $DB->get_field_sql("SELECT lot.code  FROM {local_trainingprogram} lot JOIN {tp_offerings} tpo ON tpo.trainingid = lot.id WHERE tpo.code = '$offering_code'");
                        if(!$record) {
                            $strings->offeringcode =  $offering_code;
                            $strings->offeringmappedprogramcode =  $offeringmappedprogramcode;
                            echo '<div class="local_trainingprogram_sync_error">'.get_string('offeringcodemismatched','local_trainingprogram', $strings).'</div>'; 
                            $this->errors[] =  get_string('offeringcodemismatched', 'local_trainingprogram',$strings);
                            $this->mfields[] = 'OfferingCode';
                            $this->errorcount++;
                            $eventparams = array('context' => \context_system::instance(),'objectid'=>$strings->excel_line_number,'other'=>array('name'=>'OfferingCode'));
                            $event = \local_trainingprogram\event\invalid_field::create($eventparams);// ... code that may add some record snapshots
                            $event->trigger();
                        }
        ...
        if ( array_key_exists('offeringstartdate', $excel) ) {
            ...
            if (empty($excel->offeringstartdate) && strtolower(trim($excel->trainingmethod)) != 'elearning') {
                ...
                $this->errorcount++;
                $eventparams = array('context' => \context_system::instance(),'objectid'=>$strings->excel_line_number,'other'=>array('name'=>'OfferingStartdate'));
                $event = \local_trainingprogram\event\missing_field::create($eventparams);// ... code that may add some record snapshots
                $event->trigger();
            } 
            // } else {
        ...
        if ( array_key_exists('offeringenddate', $excel) ) {
            if (empty($excel->offeringenddate) && strtolower(trim($excel->trainingmethod)) != 'elearning' ) {
                ...
                $this->mfields[] = 'OfferingEnddate';
                $this->errorcount++;
                $eventparams = array('context' => \context_system::instance(),'objectid'=>$strings->excel_line_number,'other'=>array('name'=>'OfferingEnddate'));
                $event = \local_trainingprogram\event\missing_field::create($eventparams);// ... code that may add some record snapshots
                $event->trigger();
            } /*else {
       ...
       /* if ( array_key_exists('offeringtype', $excel) ) {
        ...
        if (array_key_exists('organization', $excel) ) {
            ...
            if (!empty($excel->organization)) {
               ...
               $org = trim($excel->organization);

               $record = $DB->record_exists_sql("SELECT id FROM {local_organization} WHERE shortname = '$org'");
                if(!$record) {
       ...
       // }
        ...
        if ( array_key_exists('trainingmethod', $excel) ) {
            if (empty($excel->trainingmethod)) {
                ...
                $this->errorcount++;
                $eventparams = array('context' => \context_system::instance(),'objectid'=>$strings->excel_line_number,'other'=>array('name'=>'TrainingMethod'));
                $event = \local_trainingprogram\event\missing_field::create($eventparams);// ... code that may add some record snapshots
                $event->trigger();

            } 

            if(!empty($excel->trainingmethod) && strtolower($excel->trainingmethod)!= 'online' && strtolower($excel->trainingmethod) != 'offline' && strtolower($excel->trainingmethod) != 'elearning') {
        ...
        if ( array_key_exists('meetingtype', $excel) ) {
            ...
            if (!empty($excel->trainingmethod) && $excel->trainingmethod == 'online') {
               ...
               if (empty(trim($excel->meetingtype))) {

                    echo '<div class="local_trainingprogram_sync_error">'.get_string('meetingtypecannotbeempty','local_trainingprogram', $strings).'</div>'; 
                    $this->errors[] = get_string('meetingtypecannotbeempty','local_trainingprogram', $excel);
                    $this->mfields[] = 'MeetingType';
                    $this->errorcount++;
                    $eventparams = array('context' => \context_system::instance(),'objectid'=>$strings->excel_line_number,'other'=>array('name'=>'MeetingType'));
                    $event = \local_trainingprogram\event\missing_field::create($eventparams);// ... code that may add some record snapshots
                    $event->trigger();

                } 
                if(!empty(trim($excel->meetingtype)) && trim($excel->meetingtype) != '1' && trim($excel->meetingtype) != '2' && trim($excel->meetingtype) != '3') {
