...
class filters_form extends moodleform {
    function definition() {
        ...
        if(in_array("trackid",$filterlist)){
            $trackid          = $this->_customdata['trackid']; // this contains the data of this form
            $mform->addElement('hidden', 'trackid', $trackid);
        ...
        foreach ($filterlist as $key => $value) {
            if($value === 'useremail' || 'ltorganizationusers'){
                $filter = 'learningtracks';
            } else {
                $filter = $value;
            }
        ...
        $buttonarray[] = &$mform->createElement('button', 'filter_apply', get_string('apply','local_learningtracks'), $applyclassarray);
        $buttonarray[] = &$mform->createElement('button', 'cancel', get_string('reset','local_learningtracks'), $cancelclassarray);
