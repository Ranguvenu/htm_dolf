...
$countquery="SELECT count(le.id) FROM {local_smslogs} AS le INNER JOIN {local_notification_info} AS ni ON le.notification_infoid=ni.id where 1=1 ";

$select_query = "SELECT le.id,le.notification_infoid,le.to_userid,le.status,le.to_phonenumber,
	le.timecreated,le.sent_date,ni.notificationid, lnt.name AS notification_type
	FROM {local_smslogs} AS le
...
foreach ($result as $key => $value){
	...
	$tolocaluserrecord = $DB->get_record('local_users',['userid'=>$value->to_userid]);
        $value->to_username = ($tolocaluserrecord)? ((current_language() == 'ar') ? $tolocaluserrecord->firstnamearabic.' '.$tolocaluserrecord->middlenamearabic.' '.$tolocaluserrecord->thirdnamearabic.' '.$tolocaluserrecord->lastnamearabic  : $tolocaluserrecord->firstname.' '.$tolocaluserrecord->middlenameen.' '.$tolocaluserrecord->thirdnameen.' '.$tolocaluserrecord->lastname) :fullname($DB->get_record('user',['id'=>$value->to_userid]));


	$row = array($value->to_phonenumber,$value->to_username,$value->notification_type,$created_date,$send_date,$status,'<a href="'.$CFG->wwwroot.'/local/notifications/sms_status_details.php?id='.$value->id.'" target="_blank">'.get_string('view','local_notifications').'</a>');
	$data[] = $row;
