...
class local_notifications_external extends external_api {
    ...
    */
    public static function submit_create_notification_form($id, $contextid, $jsonformdata, $form_status) {
        ...
        $mform = new local_notifications\forms\notification_form(null, array('form_status' => $form_status,'id' => $data['id'],'moduleid'=>$data['moduleid']), 'post', '', null, true, $data);
        
        $validateddata = $mform->get_data();
        
        $lib = new \local_notifications\notification();
        if ($validateddata) {
            if ($validateddata->id > 0) {
                ...
                if($form_status == 0){
                    ...
                    $validateddata->arabic_body = $validateddata->arabic_body['text'];
    ...
    public static function managenotificationsview(
        ...
        $stable->start = $offset;
        $stable->length = $limit;
        $result_skill = $lib->notification_details($stable,$filtervalues);
            ...
            'records' => new external_multiple_structure(
                            new external_single_structure(
                                array(
                                    ...
                                    'arabic_subject' => new external_value(PARAM_RAW, 'arabic subject of notification', VALUE_OPTIONAL),
