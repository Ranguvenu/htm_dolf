...
/**
 * TODO describe file enrollment_upload
 *
 * @package    local_exams
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrollment_upload  {
    ...
    public function upload_enrollment_file($file,$sdata) {
        ...
        if(is_array($column_validation) && count($column_validation) > 0){
            ...
            $return =  '<div class="local_userapproval_sync_error">'.get_string('validsheet','local_exams',$string).'</div>'; 
            $return .= html_writer::start_tag('div', array('class'=> 'w-100 pull-left')).$OUTPUT->continue_button(new moodle_url('/local/exams/bulk_enrollment.php',['examid'=> $sdata->examid, 'profileid' => $sdata->profileid])).html_writer::end_tag('div');
           echo $return;
           return false;
        }
            ...
            if (count($this->errors) > 0) { 
               $errorcount++;
                ...
                if($userinfo->id > 0) {
                    ...
                    $masterdata->bulkenrolltype =  'exam';
                    ...
                    if(!is_siteadmin() && has_capability('local/organization:manage_organizationofficial', $systemcontext)) {
                        ...
                        $noturorganization = ((int)$sdata->organization != $traineeorganization) ? true:false;
                    } else {
                        $noturorganization = false;
                    ...
                    if(!$DB->record_exists('exam_enrollments',['examid'=>$sdata->examid,'profileid'=>$sdata->profileid,'userid'=>$userid])) {
                        $freshenrollmentusers++;
                    }
                    if($noturorganization) {
                        $noturorganizationstring = get_string('enoturorganization', 'local_exams',$updatedidnumber);
                        $return .= $OUTPUT->notification($noturorganizationstring,'error');
                    } else {
        ...
        $seatsdata =(new exams)->get_available_seats($sdata->profileid);
        if($freshenrollmentusers > $seatsdata['availableseats']) {
            $return .= $OUTPUT->notification(get_string('cantuploadmorethanavailableseats', 'local_exams',$seatsdata['availableseats']),'error');
        }
        $createdusers = base64_encode(ltrim(rtrim(implode(',',array_unique(explode(',',$registereduserid))),','),','));
        $returnurl = (COUNT(explode(',',$registereduserid)) > 1 && $createdusers && $freshenrollmentusers <= $seatsdata['availableseats']) ? new moodle_url('/local/exams/hallschedule.php', ['examid'=> $sdata->examid, 'profileid' =>$sdata->profileid,'type'=>'bulkenrollment','tuserid'=> $createdusers,'organization' =>$sdata->organization,'orgofficial' =>$sdata->orgofficial]) : new moodle_url('/local/exams/bulk_enrollment.php', ['examid'=> $sdata->examid, 'profileid' =>  $sdata->profileid]) ;
