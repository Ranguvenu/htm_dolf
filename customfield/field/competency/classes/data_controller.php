...
namespace customfield_competency;
...
class data_controller extends \core_customfield\data_controller {
    ...
    public function instance_form_definition(\MoodleQuickForm $mform) {
        ...
        $competency = $mform->addElement('autocomplete', $elementname, $this->get_field()->get_formatted_name(), $formattedoptions,array( 'placeholder' => get_string('select_competencies', 'local_questionbank')));
        $competency->setMultiple(true);
        ...
        $mform->addElement('hidden','qcategory');
    }

    // public function get_form_element_name() : string {
    //     return 'quiz_competency';
    // }

    public function instance_form_before_set_data(\stdClass $instance) {

        $qcategories = explode(',', $instance->category);
        $instance->qcategory = $qcategories[0];
    ...
    public function instance_form_save(\stdClass $datanew) {
        ...
        foreach($options as $competancy){
            $competencies = new stdClass;
            $competencies->questionbankid = $datanew->category;
            $competencies->questionid = $datanew->id;
            $competencies->competency = $competancy;
            if($DB->record_exists('local_questioncompetencies', 
                ['questionbankid' => $datanew->category, 
                 'questionid' => $datanew->id, 
                 'competency' => $competancy])){
               continue;
            }
