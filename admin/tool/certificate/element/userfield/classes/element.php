...
class element extends \tool_certificate\element {
    ...
    public function render_form_elements($mform) {
        ...
        $userfields = array(
            ...
            'arabicfullname' => get_string('arabicfullname', 'certificateelement_userfield'),
        );
    ...
    protected function get_user_field_value($user) {
        ...
        $localuser=$DB->get_record('local_users', array('userid'=>$user->id));

        if ($field === 'idnumber') {

            $localuser=$DB->get_record_sql("SELECT id,id_number FROM {local_users} 
                WHERE userid=:userid", array('userid'=>$user->id));

            if($localuser){

                $idnumber=$localuser->id_number;
            ...
            $idnumber='ID No. '.$idnumber.' <span>رقم الهوية</span>';

            return $idnumber;

        }elseif ($field === 'arabicfullname') {

          

            if($localuser){

                $arabicfullname=$localuser->firstnamearabic.' '.$localuser->middlenamearabic.' '.$localuser->thirdnamearabic.' '.$localuser->lastnamearabic;

            }else{

                $arabicfullname = fullname($user);
            ...
            return $localuser->firstname.' '.$localuser->middlenameen.' '.$localuser->thirdnameen.' '.$localuser->lastname;
