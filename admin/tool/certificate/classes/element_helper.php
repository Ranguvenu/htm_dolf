...
class element_helper {
    ...
    //eabyas added to certificate modulename
    /**
    ...
    **/
    public static function get_modulename($elementid,$user=2, $moduleinfo = false,$modulenametype) {
        global $DB;
        if($moduleinfo){
            if($modulenametype == 1){
                ...
                switch ($moduleinfo->moduletype) {
                    ...
                    case 'trainingprogram':
                        ...
                        $mname= $DB->get_field_sql('SELECT tp.name 
                                                    FROM {local_trainingprogram} as tp 
                                                    JOIN {tp_offerings} as tpo ON tpo.trainingid = tp.id 
                                                    WHERE tpo.id=:offeringid', ['offeringid' => $moduleinfo->moduleid]);

                        break;
                    case 'exams':
                        $mname = $DB->get_field('local_exams', 'exam', array('id'=>$moduleinfo->moduleid));
                        break;

                    case 'events':
                        $mname = $DB->get_field('local_events', 'title', array('id'=>$moduleinfo->moduleid));
                        break;

                    case 'learningtracks':
                        $mname = $DB->get_field('local_learningtracks', 'name', array('id'=>$moduleinfo->moduleid));
                        break;

                    case 'organizationregistration':

                        $mname = 'Organization Registration';

                        break;  

                    default:

                        $mname = 'Module name english';
                ...
                switch ($moduleinfo->moduletype) {
                    ...
                    case 'trainingprogram':
                        ...
                        $mname= $DB->get_field_sql('SELECT tp.namearabic 
                                                    FROM {local_trainingprogram} as tp 
                                                    JOIN {tp_offerings} as tpo ON tpo.trainingid = tp.id 
                                                    WHERE tpo.id=:offeringid', ['offeringid' => $moduleinfo->moduleid]);
                        ...
                        break;
                    case 'exams':
                        $mname = $DB->get_field('local_exams', 'examnamearabic', array('id'=>$moduleinfo->moduleid));
                        break;

                    case 'events':
                        $mname = $DB->get_field('local_events', 'titlearabic', array('id'=>$moduleinfo->moduleid));
                        break;

                    case 'learningtracks':
                        $mname = $DB->get_field('local_learningtracks', 'namearabic', array('id'=>$moduleinfo->moduleid));
                        break;
    ...
    //eabyas added to certificate moduleduration
    /**
     * Handles rendering the element on the pdf.
     *
     * @param \pdf $pdf the pdf object
     ...
     * @param obj $moduleinfo having information with moduletype and module
    **/
    public static function get_moduledurationdate($elementid,$user=2, $moduleinfo = false) {
        global $DB;

        $modinfo = new \stdClass();

        $modinfo->modulestartdate = time();
        $modinfo->moduleenddate = time();
        ...
        if($moduleinfo){
            ...
            switch ($moduleinfo->moduletype) {
                case 'course':
                    ...
                    $modinfo = $DB->get_record('course', array('id'=>$moduleinfo->moduleid), 'id,startdate as modulestartdate,enddate as moduleenddate');

                    break;

                case 'trainingprogram':

                    $modinfo = $DB->get_record('tp_offerings', array('id'=>$moduleinfo->moduleid), 'id,startdate as modulestartdate,enddate as moduleenddate');

                    break;

                case 'exams':

                    $modinfo = $DB->get_record('local_exams', array('id'=>$moduleinfo->moduleid), 'id,timecreated as modulestartdate');

                    break;

                case 'events':

                    $modinfo = $DB->get_record('local_events', array('id'=>$moduleinfo->moduleid), 'id,startdate as modulestartdate,enddate as moduleenddate');
