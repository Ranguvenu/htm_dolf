...
if($section == 'blocksettingfaq') {
   redirect ($CFG->wwwroot.'/blocks/faq/index.php');
}
if($section == 'blocksettingdocumentupload') {
   redirect ($CFG->wwwroot.'/blocks/documentupload/index.php');
}
...
if ($hassiteconfig && $section != "fasettings") {
...
if ($data = data_submitted() and confirm_sesskey() and isset($data->action) and $data->action == 'save-settings') {
    ...
    if( $data->section == 'hallcities' ) {
        $serializedcities = $DB->get_field('config_plugins', 'value', ['plugin' => 'local_hall', 'name' => 'hallcities']);
        $cities = unserialize($serializedcities);
        $citiescount = COUNT($cities);
        $cities[$citiescount]['city'] = $data->s_local_hall_city;
        $cities[$citiescount]['code'] = $data->s_local_hall_citycode;
        $cities[$citiescount]['region'] = $data->s_local_hall_cityregion;
        $data->s_local_hall_city = serialize($cities);
        set_config('hallcities', serialize($cities), 'local_hall');

        $data->s_local_hall_city = '';
        $data->s_local_hall_citycode = '';
        $data->s_local_hall_cityregion = '';
    }
