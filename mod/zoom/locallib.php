 ...
 * @return string
 */
function zoom_get_user_id($required = true, $overrideuser = true) {
    global $USER,$DB;
    if($overrideuser){
...
function zoom_load_meeting($id, $context, $usestarturl = true) {
    ...
    $otherhosts = $DB->get_fieldset_sql("SELECT mu.email FROM {user} mu 
    JOIN {role_assignments} rs ON mu.id = rs.userid 
    JOIN {role} rl ON rl.id = rs.roleid
    WHERE  mu.deleted = 0 AND rl.shortname IN ('to','trainer') AND rs.contextid =  $systemcontext->id");
    $alternativehosts = explode(',',implode(',', $otherhosts));
    if(is_siteadmin() || has_capability('local/organization:manage_trainingofficial',$systemcontext) || has_capability('local/organization:manage_trainer',$systemcontext)) {

        $userisrealhost = (zoom_get_user_id(false, true) === $zoom->host_id);
    } else {
        $userisrealhost = (zoom_get_user_id(false, false) === $zoom->host_id);
    }
