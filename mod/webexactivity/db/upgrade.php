...
function xmldb_webexactivity_upgrade($oldversion) {
    ...
    if ($oldversion < 2014030300) {
            ...
            $table = new xmldb_table('mod_webexactivity');
            $table->add_field('duration', XMLDB_TYPE_CHAR, '255', null, null, null, null);
            upgrade_plugin_savepoint(true, 2014030300, 'mod', 'webexactivity');
    ...
    if ($oldversion < 2020072401.19) {
        // Define field password to be added to webexactivity.
        $table = new xmldb_table('webexactivity');
        $field = new xmldb_field('meetid', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'status');
        ...
        // WebEx Activity savepoint reached.
        upgrade_mod_savepoint(true, 2020072401.19, 'webexactivity');
    }
    if ($oldversion < 2020072401.25) {

        $dbman = $DB->get_manager();

        $table = new xmldb_table('mod_webex_participants');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('meetingid', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('hostmail', XMLDB_TYPE_CHAR, '100', null, XMLDB_NOTNULL, null, null);
        $table->add_field('email', XMLDB_TYPE_CHAR, '100', null, XMLDB_NOTNULL, null, null);
        $table->add_field('jointime', XMLDB_TYPE_CHAR, '555', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('lefttime', XMLDB_TYPE_CHAR, '555', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('duration', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('siteurl', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '0');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
        upgrade_plugin_savepoint(true, 2020072401.25, 'mod', 'webexactivity');
