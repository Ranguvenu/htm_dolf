...
class webex {
    ...
    function get_participants(){
        ...
        $meetingid = 'b5ba603fbefaa4babbe1d455e5e015d7';
      
        
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://webexapis.com/v1/meetingParticipants?max=100&meetingId='.$meetingid,
          ...
          CURLOPT_HTTPHEADER => array(
            'Authorization: Bearer NzUwMTNhYjEtYzBiNi00ZjZhLTkwNDUtYTIxM2M4ZTcyY2Q5ZWJjZDA0Y2YtOTgw_PE93_d4a9cd01-f512-47b6-bcbd-ad54097f51da',
    ...
    private function proccess_recording_response($response) {
        ...
        $data = json_decode($this->get_participants());
    ...
    public function get_response($basexml, $webexuser = false) {
        ...
        if ($status) {
            ...
            if ((isset($errors['exception'])) && (($errors['exception'] === '030008'))) {
                // The WebEx User account is not activated.
                $id = optional_param('id', 0, PARAM_INT); // Course module ID.
                static $i = 0;
                $i += 1;
                if($i > 1){
                    redirect($CFG->wwwroot.'/mod/webexactivity/view.php?id='.$id, '', null, \core\output\notification::NOTIFY_SUCCESS);
                }
                redirect($CFG->wwwroot.'/mod/webexactivity/view.php?id='.$id, get_string('checkyourmail', 'mod_webexactivity'), null, \core\output\notification::NOTIFY_SUCCESS);

                throw new exceptions\user_notactive();
