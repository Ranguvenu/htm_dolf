...
class mod_attendance_renderer extends plugin_renderer_base {
    ...
    protected function render_attendance_manage_data(attendance_manage_data $sessdata) {
        // Vinod - Hiding managecontrol under the attendance table- Starts //
        //$o = $this->render_sess_manage_table($sessdata) . $this->render_sess_manage_control($sessdata);
         $o = $this->render_sess_manage_table($sessdata);
        // Vinod - Hiding managecontrol under the attendance table- Ends //
    ...
    protected function render_sess_manage_table(attendance_manage_data $sessdata) {
        ...
        foreach ($sessdata->sessions as $key => $sess) {
            ...
            $sessionstart_date =date('Y-m-d',$sess->sessdate);
            
            $current_date =date('Y-m-d');
            $sessionstartdate = date_create($sessionstart_date);
            $today = date_create($current_date);
            $daysdiff = date_diff($sessionstartdate,$today);

            $starttime = date('H:i',$sess->sessdate);
            $endtime = date("H:i",($sess->sessdate + $sess->duration));
            $currenttime = date('H:i');
          
            // Vinod - Hiding all session if the start date is not yet begain- Starts //
            ...
            $table->data[$sess->id][] = format_text($sess->description,FORMAT_HTML);

            if($sessionstart_date == $current_date && (($sess->duration > 0 && ($currenttime >= $starttime && $currenttime <= $endtime)) || ($sess->duration <= 0 && $starttime >= $currenttime))) {

                $table->data[$sess->id][] = $dta['actions'];
            } elseif($sessionstart_date < $current_date || ($sessionstart_date == $current_date && (($sess->duration > 0 &&  ($starttime < $currenttime && $currenttime > $endtime)) || ($sess->duration <= 0 && $starttime < $currenttime)))){
                if(!is_siteadmin() && !has_capability('local/organization:manage_trainer',$systemcontext) && !has_capability('local/organization:manage_trainingofficial',$systemcontext)){
                    $table->data[$sess->id][] = get_string('expired','mod_attendance');
                }
                else{
                    $table->data[$sess->id][] = $dta['actions'];
                }
                   ...
                   $table->data[$sess->id][] = get_string('notyetstarted','mod_attendance');
                }
           ...
           // Vinod - Hiding all session if the start date is not yet begain- Ends //
    ...
    private function construct_date_time_actions(attendance_manage_data $sessdata, $sess) {
        ...
        if ($sess->lasttaken > 0) {
            if (has_capability('mod/attendance:changeattendances', $sessdata->att->context)) {
                ...
                $groupidsql = 'SELECT grp.id FROM {groups} grp 
                                JOIN {tp_offerings}  tpo ON tpo.code = grp.idnumber
                                JOIN {offering_sessions} ofs ON ofs.offeringid = tpo.id
                                WHERE ofs.sessionid = :sessionid';
                $groupid = $DB->get_field_sql($groupidsql,['sessionid'=>$sess->id]);
                $groupid = ($groupid) ? (int) $groupid : $sess->groupid;

                $url = $sessdata->url_take($sess->id, $groupid);
        ...
        // Vinod - Hiding all actions except taking attendance - Starts //
        ...
        // }
         ...
         // Vinod - Hiding all actions except taking attendance - Ends  //
    ...
    protected function render_attendance_take_data(attendance_take_data $takedata) {
        ...
        foreach ($takedata->statuses as $status) {
            ...
            {
                $descriptionlang = get_string('Pfull','attendance');
            }
            if($status->description=="Late")
            {
                $descriptionlang = get_string('Lfull','attendance');
            }
            if($status->description=="Absent")
            {
                $descriptionlang = get_string('Afull','attendance');
            }
            if($status->description=="Excused")
            {
                $descriptionlang = get_string('Efull','attendance');
            }*/
            $descriptionlang = get_string($status->description,'attendance');
            $statsoutput .= "$descriptionlang = ".$sessionstats[$status->id]." <br/>";
        }

        return $controls.$statsoutput.$table;
    }
    ...
    private function construct_take_session_info(attendance_take_data $takedata) {
        ...
        $sessinfo .= html_writer::empty_tag('br');
        $sessinfo .= strip_tags(format_text($sess->description,FORMAT_HTML));
    ...
    protected function render_attendance_take_list(attendance_take_data $takedata) {
        ...
        foreach ($takedata->statuses as $st) {
            ...
            $acronymlang = get_string($st->acronym,'attendance');
            $descriptionlang = get_string($st->description,'attendance');
            
            $table->head[] = html_writer::link("#", $acronymlang, array('id' => 'checkstatus'.$st->id,
                'title' => get_string('setallstatusesto', 'attendance', $descriptionlang)));
            $table->align[] = 'center';
            $table->size[] = '20px';
    ...
    protected function render_attendance_take_grid(attendance_take_data $takedata) {
        ...
        foreach ($takedata->statuses as $st) {
            ...
            $acronym = get_string($st->acronym,'attendance');
            $descriptionlang = get_string($st->description,'attendance');
            $head[] = html_writer::link("#", $acronym, array('id' => 'checkstatus'.$st->id,
                                              'title' => get_string('setallstatusesto', 'attendance', $descriptionlang)));
            // JS to select all radios of this status and prevent default behaviour of # link.
            $this->page->requires->js_amd_inline("
                 require(['jquery'], function($) {
    ...
    protected function render_user_report_tabs(attendance_user_data $userdata) {
        ...
        //Vinod -  Hiding all tabs except this course for trainees -  Starts //
        ...
        //Vinod -  Hiding all tabs except this course for trainees -  Ends //
    ...
    private function construct_user_sessions_log(attendance_user_data $userdata) {
        ...
        foreach ($userdata->sessionslog as $sess) {
            ...
            //checking the offering sessions nazma
            ...
            $row->cells[] = format_text($sess->description,FORMAT_HTML);
            if (!empty($sess->statusid)) {
                $status = $userdata->statuses[$sess->statusid];
                $row->cells[] = format_text($status->description,FORMAT_HTML);
            ...
            // Checking the cabilities and based on the offering started or not
            if (has_capability('mod/attendance:takeattendances', $context)) {
                ...
                if(!is_siteadmin() && !has_capability('local/organization:manage_trainer',$systemcontext) && !has_capability('local/organization:manage_trainingofficial',$systemcontext)){
                    $row->cells[] = get_string('expired','mod_attendance');
                }
                else{
                    $params = array('id' => $userdata->filtercontrols->cm->id,
                    'sessionid' => $sess->id,
                   ...
                   $row->cells[] = get_string('notyetstarted','mod_attendance');
                }
    ...
    protected function get_acronym_rows(attendance_report_data $reportdata, $startwithcontrast=false) {
        ...
        foreach ($reportdata->statuses as $sts) {
            ...
            /*if($sts->acronym=="P")
            {
                $acronymlang = get_string('Pacronym','attendance');
            }
            if($sts->acronym=="L")
            {
                $acronymlang = get_string('Lacronym','attendance');
            }
            if($sts->acronym=="E")
            {
                $acronymlang = get_string('Eacronym','attendance');
            }
            if($sts->acronym=="A")
            {
                $acronymlang = get_string('Aacronym','attendance');
            }*/
            $acronymlang = get_string($sts->acronym,'attendance');
            $row2->cells[] = $this->build_header_cell($acronymlang, $contrast);
    ...
    protected function get_session_rows(attendance_report_data $reportdata, $startwithcontrast=false) {
        ...
        if ($showsessiondetails && !empty($reportdata->sessions)) {
            foreach ($reportdata->sessions as $sess) {
                ...
                if (is_null($sess->lasttaken) and has_any_capability($capabilities, $reportdata->att->context)) {
                    ...
                    if($sessionstart_date < $current_date || ($sessionstart_date == $current_date && (($sess->duration > 0 &&  ($starttime < $currenttime && $currenttime > $endtime)) || ($sess->duration <= 0 && $starttime < $currenttime))))
                    {
        ...
        if ($showsessiondetails && !empty($reportdata->sessions)) {
            foreach ($reportdata->sessions as $sess) {
                ...
                $descriptionlang = get_string($status->description,'attendance');
                    $statsoutput .= "$descriptionlang: {$status->count}<br/>";
                }
