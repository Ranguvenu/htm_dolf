...
class mod_attendance_structure {
    ...
    public function url_report($params=array()) : moodle_url {
        ...
        if(!is_siteadmin()) {
            ...
            if($this->cm->instance) {
                $codesql = 'SELECT tpo.code FROM {tp_offerings}  tpo 
                                JOIN {offering_sessions} ofs ON ofs.offeringid = tpo.id
                                JOIN {attendance_sessions} ass ON ass.id = ofs.sessionid
                                WHERE ass.attendanceid = '.$this->cm->instance.' ORDER BY ass.id DESC LIMIT 1';
                $code = $DB->get_field_sql($codesql,['attendanceid'=>$this->cm->instance]);
                $groupid = $DB->get_field_sql("SELECT grop.id FROM {groups}  grop 
                                JOIN {groups_members} grops ON grops.groupid = grop.id
    
                                WHERE grop.idnumber LIKE '%$code%' ORDER BY grop.id DESC LIMIT 1");
                $groupid = ($groupid) ? (int) $groupid : 0;
                $params = ($groupid)? array('id' => $this->cm->id,'group'=>$groupid) : array_merge(array('id' => $this->cm->id), $params);
    ...
    public function get_session_info($sessionid) {
        ...
        if (empty($this->sessioninfo[$sessionid]->description)) {
            ...
            $this->sessioninfo[$sessionid]->description = format_string(file_rewrite_pluginfile_urls($this->sessioninfo[$sessionid]->description,
