...
class block_reporttiles_reporttiles {
    ...
    public function reporttiles_icon($itemid, $blockinstanceid, $reportname) {
        ...
        if( $lang == 'en' ) {
            $reportname = str_replace(' ', '', format_string($reportname));
        } else {
            $strings = [format_string($reportname), "{mlang en}", "{mlang}", "{mlang ar}", "{mlang}", " "];
            $changedto = ["", "", "", "", "", ""];
            $reportname = str_replace($strings, $changedto, $reportname);
        }
