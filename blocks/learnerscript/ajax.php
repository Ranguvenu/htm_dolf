...
if (empty($requests['advancedfilter'])) {
	$advancedfilter = '';
} else {
	$advancedfilter = $requests['advancedfilter'];
}
...
if (empty($requests['programid'])) {
	$programid = 0;
} else {
	$programid = $requests['programid'];
}
if (empty($requests['offeringid'])) {
	$offeringid = 0;
} else {
	$offeringid = $requests['offeringid'];
}
if (empty($requests['moduleinstance'])) {
	$moduleinstance = 0;
} else {
	$moduleinstance = $requests['moduleinstance'];
}
...
if (empty($requests['trainingproviderid'])) {
	$trainingproviderid = 0;
} else {
	$trainingproviderid = $requests['trainingproviderid'];
}
if (empty($requests['typeid'])) {
	$typeid = 0;
} else {
...
$advancedfilter = optional_param('advancedfilter', $advancedfilter, PARAM_RAW);
...
$programid = optional_param('programid', $programid, PARAM_INT);
$offeringid = optional_param('offeringid', $offeringid, PARAM_INT);
$moduleinstance = optional_param('moduleinstance', $moduleinstance, PARAM_INT);
...
$trainingproviderid = optional_param('trainingproviderid', $trainingproviderid, PARAM_INT);
$typeid = optional_param('typeid', $typeid, PARAM_INT);
...
case 'generate_plotgraph':
	...
	if ($reporttype == 'table') {
		...
		foreach ($reportclass->finalreport->table->head as $key => $value) {
            ...
            if ($lang == 'ar') {
                if ($reportclass->finalreport->table->align[$i] == 'left') {
                    $reportclass->finalreport->table->align[$i] = 'right';
                } else if ($reportclass->finalreport->table->align[$i] == 'right') {
                    $reportclass->finalreport->table->align[$i] = 'left';
                }
                $align = (isset($reportclass->finalreport->table->align[$i]) || !empty($reportclass->finalreport->table->align[$i])) ? $reportclass->finalreport->table->align[$i] : 'right';
            } else {
...
case 'reportcalculations':
	...
	if ((has_capability('block/learnerscript:managereports', $context) || has_capability('block/learnerscript:manageownreports', $context) || !empty($checkpermissions)) && !empty($reportid)) {
		...
		$return = ['table' => $table, 'reportname' => format_string($reportname)];
	} else {
		$terms_data = array();
		$terms_data['error'] = true;
		$terms_data['type'] = 'Warning';
...
case 'categorycourses':
	...
	break;
case 'programofferings':
	if ($programid > 0) {
		$offerings = $DB->get_records_sql_menu("SELECT id, code FROM {tp_offerings} WHERE trainingid = :programid", ['programid' => $programid, 'visible' => 1]);
		$offerings = array('0' => 'Select offering code') + $offerings;
		}else{
			$offerings = array('' => 'Select offering code');
		}
		$return = $offerings;
	break;
case 'offeringmeetings':
	if ($offeringid > 0) {
		$meetings = $DB->get_records_sql("SELECT cm.id, m.name AS name, cm.instance
		...
		if (!empty($meetings)) {
			foreach ($meetings as $m) {
				if ($m->name == 'zoom') {
					$zoomname = $DB->get_field_sql("SELECT name FROM {zoom} WHERE id = $m->instance");
					$test[$m->id] = format_string($zoomname);
				} else if ($m->name == 'webexactivity') {
					$webexactivity = $DB->get_field_sql("SELECT name FROM {webexactivity} WHERE id = $m->instance");
					$test[$m->id] = format_string($webexactivity);
				} else if ($m->name == 'teamsmeeting') {
					$teamsmeeting = $DB->get_field_sql("SELECT name FROM {teamsmeeting} WHERE id = $m->instance");
	                $test[$m->id] = format_string($teamsmeeting);
				}
				$data = $test;
			}
			$meetings = array('0' => 'Select Virtual Activity') + $data;
		} else {
			$meetings = array('0' => 'Select Virtual Activity');
		}
	}else{
		$meetings = array('0' => 'Select Virtual Activity');
	}
...
case 'meetingdates':
	if ($offeringid > 0) {
		...
		$meetingdatess = array('' => 'Select Date');
	}
...
case 'couresevolutions':
	if ($courseid > 0) {
		$evolutions = $DB->get_records_sql_menu("SELECT id, name FROM {evolutions} WHERE course = :courseid", ['courseid' => $courseid]);
		$return = array(0 => 'Select Evolution') + $evolutions;
	} else {
		$return = array('' => 'Select Evolution');
	}
	break;
case 'designdata':
	$return = array();
	...
	if ($reportclass->config->type != 'sql') {
		if (!empty($filterdata)) {
			$filterplugins = array_unique(array_merge($reportclass->filters, $filterdata));
		} else {
	...
	if (!empty($filterplugins)) {
		foreach ($filterplugins as $p) {
			...
			// if (in_array($report->type, $pluginclass->reporttypes)) {
				...
				if ($pluginclass->selecttype == 'advanced') {
					$filtercolumns['advfiltertype'] = true;
				} else {
					$filtercolumns['advfiltertype'] = false;
				}
...
case  'learnerscriptdata':
	 $report = $DB->get_record('block_learnerscript', array('id' => $reportid));
	 if($report){
	 	$report->name = format_string($report->name);
	 	$report->summary = format_text($report->summary);
	 }
	 $return = $report;
 break;
 	case  'trainerslist':
		if ($trainingproviderid > -1) {
			$lang = current_language();
            if ($lang == 'ar') {
                $trainers = $DB->get_records_sql_menu("SELECT DISTINCT pe.userid, CONCAT(lu.firstnamearabic, ' ', lu.lastnamearabic) AS trainer FROM {program_enrollments} pe JOIN {role} r ON r.id = pe.roleid 
                    JOIN {local_users} lu ON lu.userid = pe.userid
                    WHERE pe.enrolstatus = 1 AND r.shortname = 'trainer' AND pe.trainertype =  $trainingproviderid");
            } else {
                $trainers = $DB->get_records_sql_menu("SELECT DISTINCT pe.userid, CONCAT(lu.firstname, ' ', lu.lastname) AS trainer FROM {program_enrollments} pe JOIN {role} r ON r.id = pe.roleid 
                    JOIN {local_users} lu ON lu.userid = pe.userid
                    WHERE pe.enrolstatus = 1 AND r.shortname = 'trainer' AND pe.trainertype =  $trainingproviderid");
            }
            $return = array('0' => get_string('filter_trainer', 'block_learnerscript')) + $trainers;
		} else {
			$return = array('0' => get_string('filter_trainer', 'block_learnerscript'));
		}
 		break;
 	case  'orgslist':
		if ($typeid == 1) {
			$lang = current_language();
            if ($lang == 'en') {
                $organizations = $DB->get_records_sql_menu("SELECT DISTINCT lo.id, lo.fullname FROM {local_organization} lo JOIN {tp_offerings} tp ON tp.organization = lo.id 
                    WHERE tp.type = $typeid");
            } else {
                $organizations = $DB->get_records_sql_menu("SELECT DISTINCT lo.id, lo.fullnameinarabic AS fullname FROM {local_organization} lo JOIN {tp_offerings} tp ON tp.organization = lo.id 
                    WHERE tp.type = $typeid");
            }
            $return = array('0' => get_string('filter_typeorg', 'block_learnerscript')) + $organizations;
		} else {
			$return = array('0' => get_string('filter_typeorg', 'block_learnerscript'));
		}
 		break;
}
