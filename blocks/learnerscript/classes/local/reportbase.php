...
class reportbase {
    ...
    function get_calcs($finaltable, $tablehead) {
        ...
        if (!empty($calcs)) {
            ...
            $reportid = $DB->get_field('block_learnerscript', 'type', array('id' => $this->params['reportid']), IGNORE_MULTIPLE);
            $columnstostore = array_keys($columnscalcs);
            foreach ($finaltable as $r) {
                foreach ($columnstostore as $c) {
                    if (isset($r[$c]))
                        if($reportid == 'revenue' || $reportid == 'transaction'){
                            if($r['amount'] > 0){
    ...
    if($this->reporttype == 'table'){
                ...
                foreach ($columns as $c) {
                    ...
                    if($firstrow){
                        if(isset($c['formdata']->column)) {
                            ...
                            $tablehead[$c['formdata']->column] = format_string($columnheading);
                        }
       ...
       if($this->calculations){
            ...
            foreach ($finalheadcalcs->calcdata as $key=>$head) {
                    ...
                    $calcshead1[$head] = format_string($key);
            }
            $calcsdata = array();
        ...
        if(!is_siteadmin($this->userid) && !(new ls)->is_manager($this->userid, $this->contextlevel, $this->role)) {
            ...
            if(!empty($roleincourse)){
                ...
                foreach ($roleincourse as $role) {
                    if(!empty($this->role) && (!isset($role['formdata']->contextlevel) || $role['formdata']->rolename != $this->role)) {
                        continue;
                    }
