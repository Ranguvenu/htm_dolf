			...
			if ($compclass->plugins) {
				...
				foreach ($plugins as $p) {
					...
					if (in_array($report->type, $pluginclass->reporttypes)) {
						...
						$optionsplugins[] = array('shortname'=>$p,'fullname'=> format_string(ucfirst($p)));
					}
				}
		...
		if (isset($components['plot'])) {
			foreach ($components['plot']['elements'] as $key => $value) {
				if(isset($value['formdata'])){
					if($component_data) {
						$reportcontenttypes[$value['id']] = format_string($value['formdata']->chartname);
					} else {
						$reportcontenttypes[] = array('pluginname' => $value['pluginname'], 'chartname' => format_string($value['formdata']->chartname),'chartid'=>$value['id'], 'title' => get_string($value['pluginname'], 'block_learnerscript'));
					}
				}
			}
		}
		...
		if(empty($permissions['elements'])){
            ...
            foreach ($permissions['elements'] as $p) {
            	if($p['pluginname'] == 'roleincourse') {
            		if($role == $p['formdata']->rolename && $_SESSION['ls_contextlevel'] == $p['formdata']->contextlevel) {
            			return true;
            		}
            	}
            }
    ...
    public function listofreportsbyrole($coursels = false, $statistics = false, $parentcheck = false, $reportslist = false) {
		...
		if(!empty($reportlist)) {
			...
			foreach ($reportlist as $key => $value) {
	            ...
	            $rolereports[] = ['id'=> $key, 'name' => format_string($value)];
	        }
	    }
        return $rolereports;
    }
	...
	public function strTime($values) { 
  	...
  	public function switchrole_options() {
        ...
        if (!is_siteadmin()) {
	      ...
	      $rolesql = "SELECT DISTINCT concat(r.id,'-',ctx.contextlevel)  as roleid, r.shortname
            ...
            $rolesql = "SELECT DISTINCT concat(r.id,'-',rcl.contextlevel)  as roleid, r.shortname
                   ...
                   WHERE 1 = 1 AND rcl.contextlevel = " . CONTEXT_SYSTEM;
		    $roles = $DB->get_records_sql($rolesql);
		    ksort($roles);
        ...
        foreach ($roles as $key => $value) {
            ...
            $rolefullname = $DB->get_field('role', 'name', ['shortname' => $roleshortname]);

            switch ($roleshortname) {
            	    case 'manager': $value1 = get_string('manager' , 'role'); break;
                    ...
                    default: $value1 = get_string($value->shortname, 'block_learnerscript'); break;
            }
            $contexttext = '';
            ...
            $data['roles'][] = ['roleshortname' => $roleshortname, 'rolename' => $value1,
  	...
  	public function sectodays($startdate = false, $enddate = false) { 
  		$lang= current_language(); 
  		if ($lang == 'en') {
  			$timeDiff = strtotime(userdate($enddate, '%d-%m-%Y')) - strtotime(userdate($startdate, '%d-%m-%Y'));
  			$yeardiff = $timeDiff / 86400;
			$numberDays = $yeardiff + 1;
			if ($numberDays == 1) {
				return $numberDays = $numberDays . 'day';
			} else {
				return $numberDays = $numberDays . 'days';
			}
  		} else {
  			$timeDiff = strtotime(userdate($enddate, '%d-%m-%Y')) - strtotime(userdate($startdate, '%d-%m-%Y'));
  			$yeardiff = $timeDiff / 86400;
			$numberDays = $yeardiff + 1;
			if ($numberDays == 1) {
				return $numberDays = $numberDays . 'أيام';
			} else {
				return $numberDays = $numberDays . 'أيام';
			}
  		}
