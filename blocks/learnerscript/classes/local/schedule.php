	...
	function scheduledreport_send_scheduled_report($schedule) {
		...
		foreach ($sendinguserids as $sendinguserid) {
			...
			if ($schedule->exporttofilesystem != REPORT_EXPORT) {
				...
				if (empty($attachment)) {
					$messagedetails->nodata = '<div style="background-color:#fcf8e3;color:#8a6d3b;border-color:#faebcc;width: 30%;text-align: center;padding: 5px;">'.get_string('nodataavailable','block_learnerscript').'</div>';
				}
			...
			if (empty($permissions['elements'])){
	            ...
	            foreach ($permissions['elements'] as $p) {
	            	if ($p['pluginname'] == 'roleincourse') {
	            		$roleid = $DB->get_field_sql("SELECT id FROM {role} WHERE shortname = '" . $p['formdata']->rolename . "'");
        ...
        if (!empty($permissions['elements'])) {
            foreach ($permissions['elements'] as $p) {
            	if($p['pluginname'] == 'roleincourse') {
            		...
            		$roleid = $DB->get_field('role', 'id', array('shortname' => $p['formdata']->rolename));
            		$roles[$roleid .'_'. $p['formdata']->contextlevel ] =  p['formdata']->rolename . ' at '. $contextname .' level';
            	}
            }
        } else {
