...
class querylib {
	...
	public function filter_get_users($pluginclass, $selectoption = true, $search = false, $filterdata = false, $type = false, $filterusers='', $courses = null) {
		...
		$usersoptions = array();
		if($selectoption){
			 if($pluginclass->report->type == 'evolutioncomments'){
               $usersoptions[0] = isset($pluginclass->singleselection) && $pluginclass->singleselection ? get_string('filter_trainer', 'block_learnerscript') :
	...
	public function filter_get_idnumber($pluginclass, $selectoption = true, $search = false, $type = false, $filteridnumber = '') {
		global $DB, $USER;
		$limitnum = 1;
		$searchvalue = '';
		$concatsql = "";
		...
		if ($search) {
            $searchvalue .= $search; 
            $searchsql = " AND id_number LIKE '%$search%' ";
            $limitnum = 0;
        }
		if($selectoption){
			$courseoptions[0] = isset($pluginclass->singleselection) && $pluginclass->singleselection ? get_string('filter_idnumber', 'block_learnerscript') :
								array('id' => 0, 'text' => get_string('select') . ' ' . get_string('idnumber'));
		}

		if (!empty($filteridnumber && !$search)) {
        	$concatsql .= " AND id_number = " . $filteridnumber;
        }

		$courses = $DB->get_records_select('local_users', "1 = 1 AND id_number LIKE '%$searchvalue%' " . $concatsql, array(), '', 'id_number', 0, $limitnum);

		foreach ($courses as $c) {
			if ($search) {
                $courseoptions[] = array('id' => $c->id_number, 'text' => $c->id_number);
            } else {
                $courseoptions[$c->id_number] = $c->id_number;
            }
		}
		// print_r($courses);
		return $courseoptions;
	...
	public function get_learners($useroperatorsql = '', $courseoperatorsql = ''){
		...
		$sql .= " FROM {course} c
                  ...
                  JOIN {local_users} lu ON lu.userid = ue.userid
