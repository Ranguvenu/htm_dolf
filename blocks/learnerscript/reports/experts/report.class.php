...
class report_experts extends reportbase implements report {
    ...
    public function joins() {
        $this->sql .= " JOIN {role_assignments} ra ON ra.userid = lc.userid
                        ...
                        LEFT JOIN {local_expert_request} ler ON ler.userid = lc.userid AND ler.status=2
    ...
    public function where() {
        $this->sql .= " WHERE 1 = 1  AND lc.deleted = 0 AND lc.approvedstatus = 2 AND r.shortname IN('expert') ";
        parent::where();
    }
    ...
    public function filters() {
        ...
        if(isset($this->params['filter_fieldoftraining']) && !empty($this->params['filter_fieldoftraining'])){
          ...
          $fieldoftraining =$this->params['filter_fieldoftraining'];
            $this->sql .= " AND ler.fieldoftraining LIKE '".$fieldoftraining."'";
        }
