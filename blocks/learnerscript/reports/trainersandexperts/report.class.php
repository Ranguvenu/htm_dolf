...
class report_trainersandexperts extends reportbase implements report {
    ...
    public function select() {
        ...
        if($lang == 'ar') {
           ...
           $this->sql = "SELECT DISTINCT lc.userid, $displaying_name AS fullname,lc.email as email,lc.phone1 as phone1, r.shortname AS user_role,lc.id_number AS idnumber,CASE r.shortname
            ...
            WHEN 'expert' THEN ler.fieldoftraining
            ELSE '--'
    ...
    public function joins() {
        $this->sql .= " JOIN {role_assignments} ra ON ra.userid = lc.userid
                        ...
                        LEFT JOIN {local_trainer_request} ltr ON ltr.userid = lc.userid
                        LEFT JOIN {local_expert_request} ler ON ler.userid = lcc.userid
    ...
    public function where() {
        $this->sql .= " WHERE 1 = 1  AND lc.deleted = 0 AND lc.approvedstatus = 2 AND r.shortname IN('trainer','expert')";
        parent::where();
    }
    ...
    public function filters() {
        ...
        if(isset($this->params['filter_fieldoftraining']) && !empty($this->params['filter_fieldoftraining'])){
           ...
           $this->sql .= " AND ((ltr.fieldoftraining = '".$fieldoftraining."') OR (ler.fieldoftraining = '".$fieldoftraining."'))";
        }
