...
class report_compprograms extends reportbase implements report {
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->filters = array('programfilters', 'competency', 'learningtrack', 'trainingstatus', 'type', 'typeorg', 'hallplace');
        $this->parent = true;
        ...
        $this->excludedroles = array("'manager', 'coursecreator', 'editingteacher', 'teacher', 'student', 'trainer', 'examofficial', 'em', 'co', 'expert', 'organizationofficial', 'trainee', 'cpd', 'hall_manager', 'financial_manager', 'to'");
    }
    ...
    public function filters() {
        ...
        foreach($trainingprogramfieldfilters AS $v=>$k){
            if(isset($this->params['filter_'.$v]) && $this->params['filter_'.$v] > 0){
                ...
                }else if(($v == 'namearabic' || $v == 'name' || $v == 'code' || $v == 'trainingtype' || $v == 'description' || $v == 'clevels') && !empty($this->params['filter_'.$v])){
                    $filtervalue = $this->params['filter_'.$v];
                    $this->sql .= " AND ltp.$v IN ('".$filtervalue."')";
                }
            }
    ...
    public function column_queries($columnname, $trainingprogramid, $trainingprograms = null) { 
        ...
        if ($this->params['filter_type'] > -1) {
            $test = $this->params['filter_type'];
            $typesql .= " AND pe.offeringid IN (SELECT id FROM {tp_offerings} WHERE type = $test ";
            if (!empty($this->params['filter_typeorg']) && $this->params['filter_typeorg'] > 0) {
                $test1 = $this->params['filter_typeorg'];
                $typesql .= " AND organization = $test1";
                $typesqlc .= " AND tpo.organization = $test1";

            }
            $typesql .= " )"; 
            $typesqlc .= " AND tpo.type = " . $test;
        }
        if (!empty($this->params['filter_hallplace'])) {
            $testplace = $this->params['filter_hallplace'];
            if ($this->params['filter_hallplace'] == 'clientheadquarters') {
                $typesql .= " AND pe.offeringid IN (SELECT id FROM {tp_offerings} 
                                WHERE 1 = 1 AND FIND_IN_SET('".$testplace."', halllocation))";
                $typesqlc .= " AND FIND_IN_SET('".$testplace."', tpo.halllocation) ";
            } else {
                $typesql .= " AND pe.offeringid IN (SELECT lo.id FROM {tp_offerings} lo 
                                JOIN {hall} h ON h.id = lo.halladdress
                                WHERE 1 = 1 AND h.halllocation = '".$testplace."')";
                $typesqlc .= " AND tpo.halladdress IN (SELECT id FROM {hall} WHERE halllocation = '".$testplace."') ";
            }
        }
        ...
        switch ($columnname) {
            case 'enrollments':
                ...
                $query = "SELECT COUNT(DISTINCT pe.id) AS enrollments
                            ...
                            $enrollsql  
                            $ofusql
                            WHERE 1 = 1 $where  AND lu.deleted = 0 AND lu.approvedstatus = 2 AND pe.enrolstatus = 1 $typesql ";
            break;
            case 'completions':
                $identy = 'pc.programid';
                $query = "SELECT COUNT(DISTINCT pc.id) AS completions
                            ...
                            $enrollsql 
                            $ofusql
                            WHERE 1 = 1 AND pc.completion_status != 0 $where  AND lu.deleted = 0 AND lu.approvedstatus = 2 AND pe.enrolstatus = 1 $typesql ";
            break;
            case 'offerings':
            ...
            case 'activeoffering':
                ...
                $query = "SELECT COUNT(DISTINCT tpo.id) AS offerings
                            FROM {tp_offerings} tpo 
                            $trainersql
                            WHERE 1 = 1 $where AND tpo.enddate > $timestamp $typesqlc ";
            break;
            default:
            return false;
                break;
