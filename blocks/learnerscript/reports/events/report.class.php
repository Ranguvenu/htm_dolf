...
class report_events extends reportbase implements report {

    public function __construct($report, $reportproperties) {
        global $DB;
        parent::__construct($report, $reportproperties);
        $columns = ['eventtitle', 'attendee', 'eventstartdate', 'eventenddate'];
        $this->columns = ['eventfields' => ['eventfields'], 'eventcolumns' => $columns];
        $this->components = array('columns', 'conditions', 'ordering', 'filters','permissions', 'plot');
        $this->filters = array('eventfilters');
        $this->parent = true;
        $this->orderable = array('eventtitle', 'attendee');

        $lang= current_language();
        if ($lang == 'ar') {
            $this->searchable = array('le.titlearabic');
        } else {
            $this->searchable = array('le.title');
        }

        $this->defaultcolumn = 'le.id';
        $this->excludedroles = array("'manager', 'coursecreator', 'editingteacher', 'teacher', 'student', 'trainer', 'examofficial', 'co', 'expert', 'trainee', 'to', 'competencies_official', 'cpd', 'hall_manager', 'financial_manager'");
    }
    ...
    public function select() {
        $lang= current_language();
        if ($lang == 'ar') {
            $this->sql = "SELECT DISTINCT le.id , le.titlearabic AS eventtitle, le.* ";
        } else {
            $this->sql = "SELECT DISTINCT le.id , le.title AS eventtitle, le.* ";
        }
        parent::select();
    }
    ...
    public function joins() {
        ...
        if (is_siteadmin()) {
            ...
            $this->sql .= " JOIN {tool_org_order_seats} oos ON oos.fieldid = le.id AND oos.tablename = 'local_events' AND oos.orguserid = " . $USER->id;
        }
        parent::joins();
    }
    ...
    public function filters() {
        ...
        if (!empty($this->params['filter_halladdress']) && $this->params['filter_halladdress'] > 0) {
            $this->sql .= " AND le.halladdress IN (:filter_halladdress)";
        }
    ...
    public function column_queries($columnname, $eventid, $events = null) { 
        ...
        switch ($columnname) {
            case 'attendee':
                $identy = 'lea.eventid';
                $query = "SELECT COUNT(DISTINCT lea.id) AS enrollments
                            FROM {local_event_attendees} lea 
                            JOIN {local_users} lu ON lu.userid = lea.userid
                            WHERE 1 = 1 $usersql $where  AND lu.deleted = 0 AND lu.approvedstatus = 2 AND lea.enrolstatus = 1 ";
            break;        
            default:
            return false;
                break;
