...
class report_prepostexams extends reportbase implements report {
    /**
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->parent = true;
        $this->columns = array('quizfield' => ['quizfield'] , 'prepostexamcolumns' => array('trainingprogram', 'avggrade', 'grademax', 'gradepass', 'notattemptedusers', 'inprogressusers', 'completedusers'));
        $this->components = array('columns', 'filters', 'permissions', 'plot');
        $this->courselevel = true;
        $this->orderable = array('trainingprogram', 'avggrade', 'grademax', 'gradepass', 'notattemptedusers', 'inprogressusers', 'completedusers');
        ...
        $this->excludedroles = array("'manager', 'coursecreator', 'editingteacher', 'teacher', 'student', 'trainer', 'examofficial', 'em', 'co', 'expert', 'trainee', 'competencies_official', 'cpd', 'hall_manager', 'financial_manager'");
    }
    public function count() {
        $this->sql = "SELECT COUNT(DISTINCT main.id)";
    }
    public function select() {
        $this->sql = "SELECT DISTINCT main.id, main.name AS name, ltp.name AS trainingprogram, cm.id AS activityid ";
        parent::select();
    }
    public function from() {
        $this->sql .= " FROM {quiz} as main
                        JOIN {course_modules} as cm ON cm.instance = main.id
                        JOIN {local_trainingprogram} ltp ON ltp.courseid = cm.course 
                        JOIN {tp_offerings} tpo ON ltp.id = tpo.trainingid";
    }
    ...
    public function where() {
        $this->sql .= " WHERE (tpo.prequiz IS NOT NULL OR tpo.postquiz IS NOT NULL) 
AND (tpo.prequiz = main.id OR tpo.postquiz = main.id)";
        parent::where();
    }
