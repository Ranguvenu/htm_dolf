...
class report_sql extends reportbase {
    ...
    function prepare_sql($sql) {
        ...
        if (preg_match("/%%CISISTATUS:([^%]+)%%/i", $sql, $output) && !empty($this->params['filter_cisistatus'])) {
            $status = $this->params['filter_cisistatus'];
            $replace = ' AND ' . $output[1] . ' = "'.$status.'"';
            $sql = str_replace('%%CISISTATUS:' . $output[1] . '%%', $replace, $sql);
        }        
    ...
    function get_rows() {
        ...
        if (isset($config->querysql)) {
            ...
            foreach ($columns as $c) {
                $selectedcolumns[$c['formdata']->column] = format_string($c['formdata']->column);
                $tablehead[$c['formdata']->column] = format_string($c['formdata']->columname);
