...
$meetingdate = optional_param('date', 0, PARAM_INT);
$instanceid = optional_param('instanceid', 0, PARAM_INT);

// print_r($offeringid);exit;
    ...
    foreach ($request as $key => $val) {
        if (strpos($key, 'filter_') !== false) {
            ...
            if(file_exists($CFG->dirroot . '/blocks/learnerscript/components/filters/' . $plugin . '/plugin.class.php') && !empty($val)){
	            ...
	            $selected = get_string('selectedfilter', 'block_learnerscript', ucfirst(get_string($plugin, 'block_learnerscript')));
	            $reportclass->selectedfilters[$selected] = $class->selected_filter($val, $request);
        	}
        } else if (strpos($key, 'date_') !== false) {
            $basicparamdata->{$key} = $val;
        }
    }
}
...
$_SESSION['filter_userid'] = $request['filter_userid'];
$_SESSION['filter_orguser'] = $request['filter_orguser'];
$_SESSION['filter_referenceid'] = $request['filter_referenceid'];
$_SESSION['filter_virtualactivities'] = !empty($request['instanceid']) ? $request['instanceid'] : 0;


$_SESSION['filter_trainingprogram'] = !empty($request['filter_trainingprogram']) ? $request['filter_trainingprogram'] : 0;

$_SESSION['filter_code'] = !empty($request['filter_code']) ? $request['filter_code'] : 0;

$_SESSION['filter_meetingdates'] = !empty($request['filter_meetingdates']) ? $request['filter_meetingdates'] : 0;
...
if (!$download) {
	...
	if ($report->type == 'sql' || $report->type == 'statistics') {
		echo $OUTPUT->heading(format_string($report->name)."  "."<img src='$CFG->wwwroot/pix/help.png' id='helpimg' title='Help with $reportname' alt='help' onclick = '(function(e){ require(\"block_learnerscript/report\").block_statistics_help({$report->id}) }) (event)'/>");

	} else {
		echo $OUTPUT->heading(format_string($report->name).$OUTPUT->help_icon('report_' . $report->type,
			'block_learnerscript'));
	}
