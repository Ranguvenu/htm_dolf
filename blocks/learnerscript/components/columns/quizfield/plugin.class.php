        ...
        if (isset($quizrecord->{$data->column})) {
            switch ($data->column) {
                case 'name':
                    ...
                    $quizrecord->{$data->column} = $activityicon . html_writer::link("$CFG->wwwroot/mod/$module/view.php?id=$row->activityid", format_string($quizrecord->{$data->column}),array("target" => "_blank"));
                break;
                case 'course':
