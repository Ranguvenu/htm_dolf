...
class plugin_myprogramcolumns extends pluginbase {

    public function init() {
        $this->fullname = get_string('myprogramcolumns', 'block_learnerscript');
        $this->type = 'undefined';
        $this->form = true;
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        switch ($data->column) {
            ...
            break;
            case 'time':
                $row->{$data->column} = !empty($row->{$data->column}) ? gmdate("H:i",$row->{$data->column}) : '--';
            break;
