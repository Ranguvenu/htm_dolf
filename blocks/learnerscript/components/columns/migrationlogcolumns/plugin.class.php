...
class plugin_migrationlogcolumns extends pluginbase {

    public function init() {
        $this->fullname = get_string('migrationlogcolumns', 'block_learnerscript');
        $this->type = 'undefined';
        $this->form = true;
        $this->reporttypes = array('migrationlogs');
    }
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        global $DB;
        $test = array('context' => \context_system::instance(), 'objectid' => $row->objectid, 'action' => 'field');
        $event = $row->eventname::create($test);
        switch ($data->column) {
            case 'date':
                $row->{$data->column} = !empty($row->timecreated) ? userdate($row->timecreated,get_string('strftimedatemonthabbr', 'core_langconfig')) : '--';
            break;
            case 'component':
            ...
            case 'description':
                if(!isset($row->description) && isset($data->subquery)) {
                    ...
                    $description = json_decode($row->other)->name . ' ' . $event->get_description();
                } 
