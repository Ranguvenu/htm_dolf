	...
	public function piechart($data, $series, $name, $containerid = null, $head) {
		...
		if (!empty($piedata['error']) && $piedata['error']) {
			...
			$options = ['type' => 'pie',
						...
						'title' => '' . format_string($series['formdata']->chartname) . '',
						'tooltip' => '' . format_string($tooltipvalue) . '',
						'datalabels' => '' . format_string($series['formdata']->datalabels) . '',
						'showlegend' => '' . format_string($series['formdata']->showlegend) . '',
						'serieslabel' => '' . format_string($series['formdata']->serieslabel) . '',
						'id' => $series['id'],
						'data' => $piedata,
					];
			return $options;
	...
	public function lbchart($data, $series, $name, $type, $containerid = null, $head) {
		...
		if (!empty($lbchartdata['error']) && $lbchartdata['error']) {
			...
			$options = ['type' => '' . $type . '',
						...
						'datalabels' => '' . format_string($seriesdatalabels) . '',
						'yaxistext' => format_string($yaxistext),
						'ylabel' => $head[format_string($series['formdata']->serieid)]
					];
			return $options;
		}
	}
