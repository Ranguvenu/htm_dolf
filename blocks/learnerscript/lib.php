...
function get_roles_in_context($contextlevel, $excludedroles = null){
    ...
    foreach ($roles as $r) {
        ...
        $userroles[$r->shortname] = role_get_name($r);
    }
    return $userroles;
}
...
function block_learnerscript_leftmenunode() {
    ...
    if(is_siteadmin()
        ...
        || has_capability('local/organization:manage_trainingmanager',$systemcontext) 
        ...
        || has_capability('local/organization:manage_exammanager',$systemcontext)
    ){
        $reports .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_referralcode', 'class'=>'pull-left user_nav_div reports'));
        if (is_siteadmin()) {
            $url = new moodle_url('/blocks/learnerscript/managereport.php');
        } else {
            $limit = '';
            $userrolesql = "SELECT $limit CONCAT(ra.roleid, '_',c.contextlevel) AS rolecontext, r.shortname, c.contextlevel
                        ...
                        $userrolesql .= " 1 = 1) AND c.contextlevel = 10 GROUP BY r.shortname, c.contextlevel, r.shortname $limit";
            ...
            if($roleshortname == 'examofficial' || $roleshortname == 'em' || $roleshortname == 'co'  || $roleshortname == 'organizationofficial'
            || $roleshortname == 'to' || $roleshortname == 'financial_manager' || $roleshortname == 'trainingmanager' || $roleshortname == 'exammanager'
            ){
                $url = new moodle_url('/blocks/learnerscript/reports.php?role='.$roleshortname.'&contextlevel='.$rolecontextlevel);
            } 
        }
        $label = get_string('reports','block_learnerscript');
        $referral = html_writer::link($url, '<span class="reports_icon side_menu_img_icon"></span>
        <span class="user_navigation_link_text">'.$label.'</span>',
            array('class'=>'user_navigation_link'));
        $reports .= $referral;
        $reports .= html_writer::end_tag('li');     
    }
