define(['jquery',
    ...
    function($, ajax, reportwidget, report, flatpickr, str) {
        ...
        return smartfilter = {
            ...
            programofferings: function(args) {                
                var currentprogram = $('#id_filter_trainingprogram').find(":selected").val();
                if (currentprogram > 0) {                    
                    var promise = ajax.call({
                        args: {
                            action: 'programofferings',
                            basicparam: true,
                            reporttype: args.reporttype,
                    ...
                    promise.done(function(response) {
                        ...
                        $("#id_filter_code").html(template);
                    });
                }
            },
            offeringmeetings: function(args) {
                var currentofferingcode = $('#id_filter_code').find(":selected").val();
                if (currentofferingcode > 0) {
                    var promise = ajax.call({
                        args: {
                            action: 'offeringmeetings',
                            basicparam: true,
                            reporttype: args.reporttype,
                    ...
                    promise.done(function(response) {
                        ...
                        $("#id_filter_virtualactivities").html(template);
                    });
                }
            },
            meetingdates: function(args) {                
                var currentofferingcode = $('#id_filter_code').find(":selected").val();
                var currentmoduleinstance = $('#id_filter_virtualactivities').find(":selected").val();                
                if (currentmoduleinstance > 0) {
                    var promise = ajax.call({
                        args: {
                            action: 'meetingdates',
                            basicparam: true,
                            reporttype: args.reporttype,
                            moduleinstance: args.moduleinstance,
                            offeringid: currentofferingcode
                        },
                        url: M.cfg.wwwroot + "/blocks/learnerscript/ajax.php",
                    });
                    promise.done(function(response) {
                        ...
                        $("#id_filter_meetingdates").html(template);
                    });
                }
            },
            courseEvolutions: function(args) {
                var currentcourse = $('#id_filter_courses').find(":selected").val();
                if (currentcourse > 0) {
                    var promise = ajax.call({
                        args: {
                            action: 'couresevolutions',
                            basicparam: true,
                            reporttype: args.reporttype,
                            courseid: args.courseid
                        },
                    ...
                    promise.done(function(response) {
                        ...
                        $("#id_filter_evolution").html(template);
                    });
                }
            },
            ...
            trainerslist: function(args) {
                var currentid = $('#id_filter_trainingprovider').find(":selected").val();
                if (currentid > -1) {
                    var promise = ajax.call({
                        args: {
                            action: 'trainerslist',
                    ...
                    promise.done(function(response) {
                        ...
                        $("#id_filter_trainer").html(template);
                    });
                }
            },
            orgslist: function(args) {
                var typeid = $('#id_filter_type').find(":selected").val();
                if (typeid > -1) {
                    var promise = ajax.call({
                        args: {
                            action: 'orgslist',
                    ...
                    promise.done(function(response) {
                        ...
                        $("#id_filter_typeorg").html(template);
                    });
                }
            }
        }
