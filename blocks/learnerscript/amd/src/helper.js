define(['jquery',
    ...
    function($, ajax, AjaxForms, Str, ModalFactory, ModalEvents, Ajax, notification, smartfilter) {
        return helper = {
                        ...
                        $(this).select2({
                            ...
                            ajax: {
                                data: function(params) {
                                    ...
                                    if (dataaction == 'filterusers' || dataaction == 'filtercourses' || dataaction == 'filteridnumber') {
            ...
            TpDrilldownReport: function(args) {
                var self = this;
                //Drill down report
                var lastObjectIndex = Math.max(...Object.keys(args));
;

                console.log(args);
                // $('td a').click(function(e) {
                    var url = M.cfg.wwwroot + "blocks/learnerscript/viewreport.php?id=" + args[lastObjectIndex].reportid + "&filter_trainingprogram=" + args[lastObjectIndex].filter_trainingprogram + "&filter_code=" + args[lastObjectIndex].filter_code + "&filter_meetingdates=" + args[lastObjectIndex].filter_meetingdates + "&filter_virtualactivities=" + args[lastObjectIndex].filter_virtualactivities;
                    var drillurl = self.getQueryParameters(url);
                    if (url.indexOf('viewreport.php') <= 0 || "download" in drillurl) {
                        return;
                    }
                    // e.preventDefault();
                    self.TpReportModelFromLink({ container: $(this), url: url });

                // });
            },
            TpReportModelFromLink: function(args) {
                var tableid = args.container.parents().closest('table').attr('id');
                var tablewidth = $(".currentofferingsmodel").width();
                var drillurl = this.getQueryParameters(args.url);
                var reportid = parseInt(drillurl.id);
                var drilldata = $(this).data();
                var filter = {};
                ...
                if ($("#reportcontainer" + reportid).length == 0 || $("#plotreportcontainer" + reportid).length == 0) {
                    ...
                    promise.done(function(response) {
                        ...
                        var dlg = $("#" + dialogid + reportid + "").dialog({
                            ...
                            width: tablewidth,
                            title: '',
                            appendTo: ".currentofferingsmodel",
                            modal: true,
                            position: {
                                my: "center",
                                at: "top",
                                of: ".currentofferingsmodel"
                            },
                            open: function(event) {
                                $(this).closest(".ui-dialog")
                                    .find(".ui-dialog-titlebar-close")
