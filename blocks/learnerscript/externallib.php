...
class block_learnerscript_external extends external_api {
    ...
    public static function generate_plotgraph($reportid, $courseid, $cmid, $status, $userid,
        ...
        $container, $filters, $basicparams, $columnDefs, $reportdashboard, $type=false) {
        global $DB, $PAGE, $CFG;
        ...
        if ($reportdatatable && $reporttype == 'table') {
            ...
            if(empty($reportclass->finalreport->table->data)) {
                ...
                if (!empty($reportclass->finalreport->table->head)) {
                    ...
                    $return['columnNames'] = $reportclass->finalreport->table->head;
                    $return['reporttype'] = 'table';
                    $return['emptydata'] = 0;
                } else {
                    $return['emptydata'] = 1;
            ...
            if ($report->type != 'statistics') {
                ...
                //var_dump($reportclass->finalreport->table->head); exit;
            }
        }
        if ($reporttype == 'table' && $type == 'mobile') {
            $encode = [];
            if($reporttype == 'table' && $report->type == 'statistics') {
                $encode['columnNames'] = $categorydata;
                $encode['data'] = array_combine($categorydata,$reportclass->finalreport->table->data[0]);
            } else {
                $encode['columnNames'] = $return['columnNames'];
                $record = [];
                foreach($reportclass->finalreport->table->data as $key => $value) {
                    $record[$key] = array_map('strip_tags',$value);
                    $encode['data'] = $record;
                }
                $info = json_encode($return, JSON_PRESERVE_ZERO_FRACTION);
                $encode['draw'] = json_decode($info)->data->draw;
                $encode['recordsTotal'] = json_decode($info)->data->recordsTotal;
                $encode['recordsFiltered'] = json_decode($info)->data->recordsFiltered;
                //$info = json_encode($return, JSON_PRESERVE_ZERO_FRACTION);
               // $encode['data'] = $return['data'];
            }
            $data = json_encode($encode, JSON_PRESERVE_ZERO_FRACTION);
    ...
    public static function filteridnumber_parameters() {
        return new external_function_parameters(
            array(
                'action' => new external_value(PARAM_TEXT, 'action', false),
                'maximumSelectionLength' => new external_value(PARAM_INT, 'maximum selection length to search', false),
    ...
    public static function filteridnumber($action, $maximumSelectionLength, $term, $_type, $fiterdata, $basicparamdata, $reportinstanceid, $courses) {
        global $PAGE, $DB, $CFG;
        $context = contextsystem::instance();
        $PAGE->set_context($context);
        $search = $term;
        ...
        $pluginclass->report->type = 'custom';
        $pluginclass->reportclass = $reportclass;
        $courseoptions = (new \block_learnerscript\local\querylib)->filter_get_idnumber($pluginclass, true, $search, $_type);
        $terms_data = array();
        $terms_data['total_count'] = sizeof($courseoptions);
        $terms_data['incomplete_results'] = false;
        $terms_data['items'] = $courseoptions;
    ...
    public static function filteridnumber_returns() {
        return new external_value(PARAM_RAW, 'data');
    }
    public function reportslist_parameters() {
        return new external_function_parameters(
            array(
                'type' => new external_value(PARAM_RAW, 'type', false),
            )
        );
    }
    public function reportslist($type) {
        global $CFG, $DB, $USER;
       ...
       // $rolelist = (new ls)->get_currentuser_roles();
        if (!is_siteadmin()) {
            ...
            $userrolesql = "SELECT $limit CONCAT(ra.roleid, '_',c.contextlevel) AS rolecontext, r.shortname, c.contextlevel
                            ...
                            WHERE 1 = 1 AND ra.userid = :userid AND c.contextlevel NOT IN (40,50) AND (";
        ...
        $role = $roleshortname;
        $lscontextlevel = $rolecontextlevel;
        $reportlist = $DB->get_records_sql(" SELECT id, name, type FROM {block_learnerscript} WHERE global = 1 AND visible = 1" );
        $rolereports = array();  
        if(!empty($reportlist)) {
            $properties = new stdClass();
            ...
            foreach ($reportlist as $list) {
                if(!empty($role) && !empty($lscontextlevel) && $lscontextlevel != 50) {
                    $reportcomponents = $DB->get_field('block_learnerscript', 'components', array('id' => $list->id));
                    ...
                    if(empty($permissions['elements'])){
                        ...
                        foreach ($permissions['elements'] as $p) {
                            if($p['pluginname'] == 'roleincourse') {
                                if($role == $p['formdata']->rolename && $lscontextlevel == $p['formdata']->contextlevel) {
                                    $rolereports['id'] = $list->id;
                                    $rolereports['name'] =  format_string($list->name);
                                    $rolereports['type'] = get_string($list->type,'block_learnerscript');
                                    $data['data'][] = $rolereports;
                                }
                            }
                        }
                    }
        ...
        return ['role' => $role, 'data' => $return];
    }
    public function reportslist_returns() {
        return new external_single_structure([
            'role' => new external_value(PARAM_RAW, 'role', VALUE_OPTIONAL),
    ...
    public static function get_reports($reportid, $reporttype, $type=false) {
        global $DB, $PAGE, $CFG;
        $ls = new ls();
        $PAGE->set_context(contextsystem::instance());
        $learnerscript = $PAGE->get_renderer('block_learnerscript');
        ...
        if ($reportdatatable && $reporttype == 'table') {
            ...
            if(empty($reportclass->finalreport->table->data)) {
                ...
                $return['columnName'] = $reportclass->finalreport->table->head;
                if (!empty($reportclass->finalreport->table->head)) {
                    $tablehead = (new ls)->report_tabledata($reportclass->finalreport->table);
                    $return = array();
                    $columns = [];
                    foreach ($reportclass->finalreport->table->data as $key => $value) {
                        $data[$key] = array_values(array_map('strip_tags',$value));
                    }
                    $i=0;
                    foreach($reportclass->finalreport->table->head as $key => $value) {
                        $columns[$i] = $value;
                    ...
                    $reporttitle = get_string('report_' . $report->type, 'block_learnerscript');
                    //$return['reportname'] = (new ls)->get_reporttitle($reporttitle, []);
                    $return['columnNames'] = $columns; //$reportclass->finalreport->table->head;
                    $return['data'] = $data;   
                }
            }
        } else{
    ...
    public static function get_reports_returns() {
        return new external_single_structure([
            // 'recordsTotal' =>  new external_value(PARAM_RAW, 'recordsTotal', VALUE_OPTIONAL),
            'columnNames' => new external_multiple_structure( 
                new external_value(PARAM_TEXT, 'columnNames',), 'Array of course module IDs' , '', VALUE_OPTIONAL),
            // 'data' => new external_multiple_structure( 
            //     new external_value(PARAM_TEXT, 'data'), 'Array of course module IDs' ), '', VALUE_OPTIONAL,
        

             'data' => new external_multiple_structure(
                 new external_multiple_structure(
                     new external_value(PARAM_TEXT, 'data'), 'Array of course module IDs' ,
                      '', VALUE_OPTIONAL))
