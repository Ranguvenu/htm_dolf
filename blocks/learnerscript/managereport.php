...
if ($reports) {
    ...
    $table->width = "100%";
    $table->head = array(get_string('name', 'block_learnerscript'),  get_string('type', 'block_learnerscript'), get_string('actions', 'block_learnerscript'), get_string('download', 'block_learnerscript'));
    ...
    foreach ($reports as $r) {
        ...
        $editcell .= '<a title="' . $strdelete . '"  href="editreport.php?id=' . $r->id . '&amp;delete=1"><img src="' . $OUTPUT->image_url('/t/delete') . '" class="iconsmall" alt="' . $strdelete . '" /></a>';
        ...
        $table->data[] = array('<a href="viewreport.php?id=' . $r->id . '">' . format_string($r->name) . '</a>', get_string('report_' . $r->type, 'block_learnerscript'), $editcell, $download);
    }
