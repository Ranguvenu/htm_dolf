...
class core_user_external extends external_api {
    ...
    public static function get_users_by_field($field, $values) {
        ...
        foreach ($users as $user) {
            ...
            $roleinfo = $DB->get_record_sql('SELECT rl.id,rl.shortname,rl.name FROM {role} rl
                                              JOIN {role_assignments} rol ON rol.roleid = rl.id
                                              WHERE rol.userid = :userid 
                                              AND rol.contextid = :contextid',['userid'=>$userdetails['id'],'contextid'=>$context->id]);          
            $userdetails['role']['id'] = $roleinfo->id;
            $userdetails['role']['shortname'] = $roleinfo->shortname;
            $userdetails['role']['name']= $roleinfo->name;


            $useraditionalinfo = $DB->get_record('local_users',['userid'=>$userdetails['id']]);

            $userdetails['localinfo']['firstnamearabic']  = $useraditionalinfo->firstnamearabic;
            $userdetails['localinfo']['lastnamearabic']  = $useraditionalinfo->lastnamearabic;
            $userdetails['localinfo']['mobile']  = $useraditionalinfo->phone1;
            $userdetails['localinfo']['gender']  =  (is_null($useraditionalinfo->gender) || $useraditionalinfo->gender == 0)? null : (($useraditionalinfo->gender == 1)? get_string('male','local_userapproval'):get_string('female','local_userapproval'));
            $userdetails['localinfo']['city']  =  (is_null($useraditionalinfo->city))? null : $useraditionalinfo->city;
            $countries = get_string_manager()->get_list_of_countries();
            $userdetails['localinfo']['country']  =  (is_null($useraditionalinfo->country))? null : $countries[$useraditionalinfo->country];
            if ($useraditionalinfo->id_type == 1) {
               $id_type=get_string('id','local_userapproval');
            } elseif($useraditionalinfo->id_type == 2) {
                $id_type=get_string('passport','local_userapproval');
            } elseif($useraditionalinfo->id_type == 3) {
                $id_type=get_string('saudiid','local_userapproval');
            } else {
                $id_type=get_string('residentialid','local_userapproval');
            }
            $userdetails['localinfo']['id_type']  =  (is_null($useraditionalinfo->id_type) || $useraditionalinfo->id_type == 0 )? null : $id_type;

            $userdetails['localinfo']['id_number']  =  (is_null($useraditionalinfo->id_number) || $useraditionalinfo->id_number == 0 )? null : $useraditionalinfo->id_number;

            $userdetails['localinfo']['organization']['id']  = (is_null($useraditionalinfo->organization) || empty($useraditionalinfo->organization))? 0 : (int)$useraditionalinfo->organization;
            $userdetails['localinfo']['organization']['name']  = (is_null($useraditionalinfo->organization) || empty($useraditionalinfo->organization))? '' : $DB->get_field('local_organization','fullname',['id'=>(int)$useraditionalinfo->organization]);
            $userdetails['localinfo']['organization']['namearabic']  = (is_null($useraditionalinfo->organization) || empty($useraditionalinfo->organization))? '' : $DB->get_field('local_organization','fullnameinarabic',['id'=>(int)$useraditionalinfo->organization]);

            $userdetails['localinfo']['sector']['id']  = (is_null($useraditionalinfo->sector) || empty($useraditionalinfo->sector))? 0 : (int)$useraditionalinfo->sector;
            $userdetails['localinfo']['sector']['name']  = (is_null($useraditionalinfo->sector) || empty($useraditionalinfo->sector))? '' : $DB->get_field('local_sector','title',['id'=>(int)$useraditionalinfo->sector]);
            $userdetails['localinfo']['sector']['namearabic']  = (is_null($useraditionalinfo->sector) || empty($useraditionalinfo->sector))? '' : $DB->get_field('local_sector','titlearabic',['id'=>(int)$useraditionalinfo->sector]);

             $userdetails['localinfo']['segment']['id']  = (is_null($useraditionalinfo->segment) || empty($useraditionalinfo->segment))? 0 : (int)$useraditionalinfo->segment;
            $userdetails['localinfo']['segment']['name']  = (is_null($useraditionalinfo->segment) || empty($useraditionalinfo->segment))? '' : $DB->get_field('local_segment','title',['id'=>(int)$useraditionalinfo->segment]);
            $userdetails['localinfo']['segment']['namearabic']  = (is_null($useraditionalinfo->segment) || empty($useraditionalinfo->segment))? '' : $DB->get_field('local_segment','titlearabic',['id'=>(int)$useraditionalinfo->segment]);


             $userdetails['localinfo']['jobfamily']['id']  = (is_null($useraditionalinfo->jobfamily) || empty($useraditionalinfo->jobfamily))? 0 : (int)$useraditionalinfo->jobfamily;
            $userdetails['localinfo']['jobfamily']['name']  = (is_null($useraditionalinfo->jobfamily) || empty($useraditionalinfo->jobfamily))? '' : $DB->get_field('local_jobfamily','familyname',['id'=>(int)$useraditionalinfo->jobfamily]);
            $userdetails['localinfo']['jobfamily']['namearabic']  = (is_null($useraditionalinfo->jobfamily) || empty($useraditionalinfo->jobfamily))? '' : $DB->get_field('local_jobfamily','familynamearabic',['id'=>(int)$useraditionalinfo->jobfamily]);


             $userdetails['localinfo']['jobrole']['id']  = (is_null($useraditionalinfo->jobrole) || empty($useraditionalinfo->jobrole))? 0 : (int)$useraditionalinfo->jobrole;
            $userdetails['localinfo']['jobrole']['name']  = (is_null($useraditionalinfo->jobrole) || empty($useraditionalinfo->jobrole))? '' : $DB->get_field('local_jobrole_level','title',['id'=>(int)$useraditionalinfo->jobrole]);
            $userdetails['localinfo']['jobrole']['namearabic']  = (is_null($useraditionalinfo->jobrole) || empty($useraditionalinfo->jobrole))? '' : $DB->get_field('local_jobrole_level','titlearabic',['id'=>(int)$useraditionalinfo->jobrole]);

            $userdetails['localinfo']['bannerimage']  = (is_null($useraditionalinfo->bannerimage) || $useraditionalinfo->bannerimage == 0)? '' : bannerimage_url($useraditionalinfo->bannerimage);
            $userdetails['localinfo']['certificates']  = (is_null($useraditionalinfo->certificates) || empty($useraditionalinfo->certificates))? 0 : (int)$useraditionalinfo->certificates;;
            $userdetails['localinfo']['linkedinprofile']  = $useraditionalinfo->linkedinprofile;

            $staticLang='ar';
            if($staticLang){
               $SESSION->lang =$staticLang;
               $userdetails['localinfo']['genderarabic']  =  (is_null($useraditionalinfo->gender) || $useraditionalinfo->gender == 0)? null : (($useraditionalinfo->gender == 1)? get_string('male','local_userapproval'):get_string('female','local_userapproval'));

               // $userdetails['localinfo']['countryarabic']  =  (is_null($useraditionalinfo->country))? null : $countries[$useraditionalinfo->country];

               // $userdetails['localinfo']['idtypearabic']  =   (is_null($useraditionalinfo->id_type) || $useraditionalinfo->id_type == 0 )? null : $id_type;
            }
    ...
    public static function user_description($additionalfields = array()) {
        $userfields = array(
            ...
            ), 'User Role', VALUE_OPTIONAL),
             'localinfo' => 
            new external_single_structure(
                array(
                    'firstnamearabic'  => new external_value(PARAM_RAW, 'firstnamearabic'),
                    'lastnamearabic' => new external_value(PARAM_RAW, 'lastnamearabic'),
                    'mobile'  => new external_value(PARAM_INT, 'mobile'),
                    'gender'  => new external_value(PARAM_TEXT, 'gender'),
                    'genderarabic'  => new external_value(PARAM_RAW, 'genderarabic'),
                    'city'  => new external_value(PARAM_TEXT, 'city'),
                    'country'  => new external_value(PARAM_TEXT, 'country'),
                    //'countryarabic'  => new external_value(PARAM_RAW, 'countryarabic'),
                    'id_type'  => new external_value(PARAM_TEXT, 'id_type'),
                    //'idtypearabic'  => new external_value(PARAM_RAW, 'idtypearabic'),
                    'id_number'  => new external_value(PARAM_TEXT, 'id_number'),
                    'organization' => 
                        new external_single_structure(
                            array(
                                'id'  => new external_value(PARAM_INT, 'ID'),
                                'name' => new external_value(PARAM_RAW, 'Name'),
                                'namearabic' => new external_value(PARAM_RAW, 'Name Arabic'),
                    ),'User organization', VALUE_OPTIONAL),
                    'sector' => 
                        new external_single_structure(
                            array(
                                'id'  => new external_value(PARAM_INT, 'ID'),
                    ...
                    'jobrole' => 
                        new external_single_structure(
                            array(
                                ...
                                'name' => new external_value(PARAM_RAW, 'Role Name'),
                                'namearabic' => new external_value(PARAM_RAW, 'Name Arabic'),
                    ),'User jobrole', VALUE_OPTIONAL),   
                    'bannerimage'  => new external_value(PARAM_RAW, 'bannerimage'),
                    'certificates'  => new external_value(PARAM_RAW, 'certificates'),
                    'linkedinprofile'  => new external_value(PARAM_RAW, 'linkedinprofile')
